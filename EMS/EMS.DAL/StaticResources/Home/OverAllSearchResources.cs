...
namespace EMS.DAL.StaticResources
{
    public class OverAllSearchResources
    {
        ...
        /// 区域最近31天用能
        /// </summary>
        public static string RegionLast31DaySQL = @" SELECT Region.F_RegionID AS ID,Region.F_RegionName AS Name 
		                                                    ,DayResult.F_StartDay AS 'Time'
        ...
        /// 支路-月份环比
        /// </summary>
        public static string CircuitMomMonthSQL = @" SELECT t1.F_MeterID AS ID,T1.F_CircuitName AS Name,t1.F_Value AS Value,t2.F_Value AS LastValue ,
	                                                        (t1.F_Value - t2.F_Value) AS DiffValue
	                                                        ,case when t2.F_Value = 0 then NULL ELSE (t1.F_Value - t2.F_Value)/t2.F_Value*100 END AS Rate
                                                        FROM (
		                                                        SELECT DayResult.F_MeterID,Circuit.F_CircuitName,SUM(DayResult.F_Value) AS F_Value 
		                                                        FROM T_MC_MeterDayResult DayResult
		                                                        ...
		                                                        GROUP BY DayResult.F_MeterID,Circuit.F_CircuitName,DATEADD(MM,DATEDIFF(MM,0,DayResult.F_StartDay),0)
		                                                        ) T1
                                                        INNER JOIN 
        ...
        /// 支路-月份同比
        /// </summary>
        public static string CircuitCompareMonthSQL = @" SELECT t1.F_MeterID AS ID,T1.F_CircuitName AS Name,t1.F_Value AS Value,t2.F_Value AS LastValue ,
	                                                        (t1.F_Value - t2.F_Value) AS DiffValue
	                                                        ,case when t2.F_Value = 0 then NULL ELSE (t1.F_Value - t2.F_Value)/t2.F_Value*100 END AS Rate
                                                        FROM (
		                                                        SELECT DayResult.F_MeterID,Circuit.F_CircuitName,SUM(DayResult.F_Value) AS F_Value 
