	...
	function _compare(){
		...
		function initDateTime(){
			...
			$("#calendarbox").change(function(event) {
				...
				getDataFromServer("/api/DeptCompare","buildId="+buildId+"&energyCode="+
					$(".btn-solar-selected").attr('value')+"&departmentID="+node[0].id+"&date="+date);
			});
		}
		...
		function showBuilds(data){
			...
			$("#buildinglist").change(function(event) {
				...
				$.cookie("buildId",buildId,{path:'/'})
				 //console.log($("#daycalendarBox").val());
				getDataFromServer("/api/DeptCompare","buildId="+buildId+"&date="+$("#calendarbox").val());
			});
		};
		...
		function showEnergys(data){
			...
			$.each(data.energys, function(key, val) {
				...
				switch(val.energyItemCode){
					case "01000":
						$("#se_CompareBtns").append('<acronym title="电"><button id="'+val.energyItemCode+'" class="btn btn-elc" value="01000" style="width: 20px; height: 20px;" unit="'+val.energyItemUnit+'" type="button"></button></acronym>')
					break;
					case "02000":
						$("#se_CompareBtns").append('<acronym title="水"><button id="'+val.energyItemCode+'" class="btn btn-water" value="02000" style="width: 20px; height: 20px;" unit="'+val.energyItemUnit+'" type="button"></button></acronym>')
					break;
					case "13000":
			...
			$("#se_CompareBtns button").click(function(event) {//为能源按钮绑定click事件，进行数据加载
				var $current = $(this);
				...
				if(isNotRepeat){
					//发送请求
					getDataFromServer("/api/DeptCompare","buildId="+$("#buildinglist").val()+"&energyCode="+
		...
		function showTreeview(data){
			...
			EMS.DOM.initTreeview(data.treeView,$("#treeview"),{
				...
				onNodeSelected:function(event,node){
					...
					getDataFromServer("/api/DeptCompare","buildId="+buildId+
						"&energyCode="+$(".btn-solar-selected").attr('value')+
						"&departmentID="+id+"&date="+$("#calendarbox").val());
				}
			});
