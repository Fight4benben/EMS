	...
	function _compare(){
		...
		function initDateTime(){
			...
			$("#calendarbox").change(function(event) {
				...
				getDataFromServer("/api/RegionCompare","buildId="+buildId+"&energyCode="+
					$(".btn-solar-selected").attr('value')+"&regionID="+node[0].id+"&date="+date);
			});
		}
		...
		function showEnergys(data){
			...
			$("#se_CompareBtns button").click(function(event) {//为能源按钮绑定click事件，进行数据加载
				...
				if(isNotRepeat){
					//发送请求
					getDataFromServer("/api/RegionCompare","buildId="+$("#buildinglist").val()+"&energyCode="+
		...
		function showTreeview(data){
			...
			EMS.DOM.initTreeview(data.treeView,$("#treeview"),{
				...
				onNodeSelected:function(event,node){
					...
					getDataFromServer("/api/RegionCompare","buildId="+buildId+
						"&energyCode="+$(".btn-solar-selected").attr('value')+
						"&regionID="+id+"&date="+$("#calendarbox").val());
				}
			});
