var Home = (function () {
	function _home() {
		...
		function showBuildInfo(data) {
			...
			$("#TransCnt h4").html(currentBuild.transCount + '台');
			$("#InstallCap h4").html(currentBuild.installCapacity + 'kVA');
			$("#OperateCap h4").html(currentBuild.operateCapacity + 'kVA');
			$("#metersTotal h4").html(currentBuild.designMeters + '只');
		...
		function showCompareEnergyButtonAndShowLineAndCompare(data) {
			...
			$.each(data.compareValues, function (key, val) {
				...
				energyArray.push({
					code: val.energyItemCode, compareValues: { today: val.todayValue, yesterday: val.yesterdayValue },
